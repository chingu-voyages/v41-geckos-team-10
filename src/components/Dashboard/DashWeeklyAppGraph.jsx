  ...
  return (
    <div className='graph'>
      <div className='graph_container'>
        <div className='graph_text-container'>
          {graphBarArr.map((bar) => {
            return (
              <p className={`graph_text left-${bar.spacing}`}>{bar.day}</p>
            );
          })}
        </div>
        <div className='graph_line-container'>
          {graphLineSpacingArr.map((line) => {
            return <div className={`graph_line bottom-${line}`} />;
          })}

          {graphBarArr.map((bar) => {
            return (
              <div
                className={`graph_bar  h-${bar.percent}/100 left-${bar.spacing}`}
              >
                {bar.percent}
              </div>
            );
